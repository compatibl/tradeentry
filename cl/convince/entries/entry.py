...
class Entry(EntryKey, RecordMixin[EntryKey], ABC):
    ...
    """Value generated by AI in string format."""

    approved: str | None = None
    """Value approved by the user in string format."""
    ...
    def run_approve(self) -> None:
        ...
        # Normalize both proposed and approved string
        self.proposed = CharUtil.normalize_chars(self.proposed)
        self.approved = CharUtil.normalize_chars(self.approved)

        # Set approved value based on proposed if not specified directly
        if self.approved is None:
            if self.proposed is not None:
                # Approved is not set but proposed is, accept proposed
                self.approved = self.proposed
            else:
                # If proposed is also not set, user message
                raise UserError(f"Proposed value is empty, run 'Propose' handler to generate.\n"
