...
class RecordUtil:
    """Util for record conversion to ui format."""
    ...
    def to_record_dict(node):  # TODO: Apply type hints
        ...
        if node_type in primitive_types:
            ...
            return [RecordUtil.to_record_dict(v) for v in node if v is not None]
            ...
            result = {k: RecordUtil.to_record_dict(v) for k, v in node_dict.items() if v is not None}
            return result
        else:
            return node
    ...
    def to_legacy_dict(node: Dict[str, Any] | List[Dict[str, Any]] | str) -> Dict[str, Any] | List[Dict[str, Any]] | str:
        ...
        if isinstance(node, dict):
            ...
            result = {
                pascalize(k.removesuffix("_")): RecordUtil.to_legacy_dict(v) for k, v in node.items() if v is not None
            }
            return result
        elif isinstance(node, list):
            # Skip nodes that have the value of None
            return [RecordUtil.to_legacy_dict(v) for v in node if v is not None]
    ...
    @staticmethod
    def to_legacy_table(record) -> Dict[str, Any]:
        """Serialize record to dictionary of selected table fields."""
        
        record_dict = {
            slot: v for slot in record.__slots__
            if type(v := getattr(record, slot)) in primitive_types
            or (is_key(v))
            or (isinstance(v, dict) and v.get('_enum') is not None)
        }

        return RecordUtil.to_legacy_dict(RecordUtil.to_record_dict(record_dict))
